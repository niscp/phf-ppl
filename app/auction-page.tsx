...
function AdminAuction({ snapshot, refresh }: { snapshot: AuctionSnapshot; refresh: () => Promise<void> }) {
  ...
  const [rules, setRules] = useState({ base: "1", increment: "1", threshold: "28", incrementAbove: "1", minSquad: "14", squad: "15", unit: "CR" });
  const [auctions, setAuctions] = useState<AuctionInstance[]>([]);
  const [newAuction, setNewAuction] = useState({ name: "Season 5 rehearsal", kind: "demo", purse: "28", base: "1", increment: "1", threshold: "28", incrementAbove: "1", minSquad: "14", maxSquad: "15" });
