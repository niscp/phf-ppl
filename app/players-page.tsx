...
export default function PlayersPage() {
  ...
  return <main className="roster-page">
    ...
    <section className="roster-directory" aria-label="Registered players">
      <div className="roster-intro"><div><p className="roster-eyebrow">The roster</p><h2>Meet the players.</h2></div><p>{verifiedCount} verified CricHeroes career records from {profileCount} submitted profile links. These are career totals, not PHF-only figures. Unmatched profiles remain blank.</p></div>
      ...
      {visible.length ? <div className="roster-grid">{visible.map((player) => <article className="roster-card" key={player.id}>
        ...
        <div className="roster-card-body">
          ...
          {player.cricheroesUrl ? <a className="roster-profile-link" href={player.statsSource ?? player.cricheroesUrl} target="_blank" rel="noopener noreferrer">{player.statsSource ? "View career stats on CricHeroes" : "Open CricHeroes profile"} <span aria-hidden="true">↗</span></a> : <span className="roster-no-profile">CricHeroes profile not supplied</span>}
        </div>
      ...
      <p className="roster-source-note">Player names, submitted photos and roles come from the Season 5 registration form. Verified stats come from each linked CricHeroes career record; they are not Season 5 or PHF-only totals. An unavailable message never means zero runs or wickets. No contact or payment details are published.</p>
    </section>
