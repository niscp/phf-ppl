...
2. In the Supabase SQL Editor, apply [`202609180001_season5_auction.sql`](../supabase/migrations/202609180001_season5_auction.sql) and then [`202609190001_live_bidding_and_logos.sql`](../supabase/migrations/202609190001_live_bidding_and_logos.sql). The first creates the auction tables, read-only public policies, audited auctioneer functions, and realtime updates. The second adds logos and the bid-reset function.
...
- Add official team logo URLs in the auctioneer console when available. Until then, a team-name crest appears; no logo is invented. The public `/teams.html` page shows the six teams and their squads in real time.
...
- During bidding, clicking a team card records the next agreed increment and makes it the leading team. Its displayed available purse falls provisionally, but `spent` does not change. If another team bids or the auctioneer resets all bids for the current player, that provisional hold disappears. Only **Sold** deducts the winning amount and assigns the player to that squad. Resetting bids does not undo a sale.
